...
from flexolmo_analysis.toolkit.prev_analysis.plots import plot_expert_combination_upset
from flexolmo_analysis.toolkit.plotting.style import (
    FONT_SIZES,
    apply_axis_text_style,
    dataset_display_name,
    expected_num_experts_for_model,
    expert_colors_for_model,
...
def save_usage_bar_plot(usage, path: str | Path, model_name: str, dataset_name: str, title: str = "Aggregate Expert Usage"):
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    expected = expected_num_experts_for_model(model_name)
    if expected is not None and len(usage) > expected:
        usage = usage[:expected]

    fig, ax = plt.subplots(figsize=(8, 4))
    expert_indices = list(range(len(usage)))
    labels = expert_tick_labels_for_model(model_name, len(usage), multiline=True)
    colors = expert_colors_for_model(model_name, len(usage))
    ax.bar(expert_indices, usage, color=colors)
    style_axis_labels(ax, "Expert", "Normalized usage")
    style_axis_title(ax, f"{model_display_name(model_name)} | {dataset_display_name(dataset_name)}", pad=3)
...
def save_coactivation_heatmap(matrix, path: str | Path, model_name: str, dataset_name: str, title: str = "Aggregate Coactivation Matrix", annotate: bool = True):
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    labels = expert_tick_labels_for_model(model_name, len(matrix), multiline=True)
    heatmap = sns.heatmap(
        matrix,
        cmap="viridis",
        ax=ax,
    ...
    style_axis_labels(ax, "Expert", "Expert")
    style_heatmap_ticklabels(ax, labels, labels, x_rotation=35)
    if heatmap.collections and heatmap.collections[0].colorbar is not None:
        style_colorbar(heatmap.collections[0].colorbar, "Normalized Co-activation")
    short_title = title if title.startswith("L") else "Aggregate"
    style_axis_title(ax, f"{short_title} | {model_display_name(model_name)}", pad=3)
    style_suptitle(fig, dataset_display_name(dataset_name), y=0.97)
    fig.savefig(path, dpi=200, bbox_inches="tight")
    plt.close(fig)
...
def save_layerwise_upset_plots(
    ...
    for layer_idx in range(num_layers):
        ...
        if layer_combinations:
            path = output_dir / f"{stem}_{layer_idx}.png"
            generated = plot_expert_combination_upset(
                combination_counts=layer_combinations,
                path=path,
                title=f"{model_display_name(model_name)} | {dataset_display_name(dataset_name)} | L{layer_idx} expert combinations",
                model_name=model_name,
            )
            if generated:
                paths.append(path)
...
def plot_routing_outputs(
    ...
    save_usage_bar_plot(routing_aggregate["usage"], usage_plot_path, model_name=model_name, dataset_name=dataset_name)
    save_coactivation_heatmap(
        routing_aggregate["coactivation_matrix"],
        coactivation_plot_path,
        model_name=model_name,
        dataset_name=dataset_name,
    )
