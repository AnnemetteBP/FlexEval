...
from flexolmo_analysis.toolkit.core.routing_diagnostics import (
    compute_all_metrics,
    compute_offdiagonal_ratio,
    normalize_coactivation_counts,
)
from flexolmo_analysis.toolkit.adapters.flex_olmo import FlexOlmoAdapter
from flexolmo_analysis.toolkit.pipelines.flex_olmo import analyze_flex_olmo_routing, restricted_expert_mode
from flexolmo_analysis.toolkit.pipelines.flex_olmo_eval import FlexOlmoEvalRunSpec
...
def tokenize_prompt(
    ...
    if input_ids is None or input_ids.ndim != 2 or input_ids.shape[-1] == 0:
        ...
        raise ValueError(
            ...
            f"(tokenizer_class={tokenizer.__class__.__name__}, max_length={max_length}, "
            f"add_special_tokens={add_special_tokens}, prompt_preview={prompt_preview!r})."
        )
...
def analyze_prompt_example(
    ...
    if inputs["input_ids"].shape[-1] == 0:
        raise ValueError(
            "Empty tokenized prompt reached routing analysis "
            f"for example_id={example.get('example_id')} dataset={example.get('dataset_name')}."
        )
    ...
    with routing_context:
        ...
        if capture_output_token_ids:
            ...
            qa_scores = qa_score_bundle(
                prediction=predicted_output_text,
    ...
    return {
        ...
        "qa_scores": qa_scores if capture_output_token_ids else {},
...
def summarize_routing_records(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for run_label, run_records in by_run.items():
        ...
        mean_qa_f1 = (
            sum(float(record.get("qa_scores", {}).get("f1", 0.0)) for record in run_records) / total
            if total
            else 0.0
        )
        mean_qa_em = (
            sum(float(record.get("qa_scores", {}).get("em", 0.0)) for record in run_records) / total
            if total
            else 0.0
        )
        ...
        summaries.append(
            {
                ...
                "mean_qa_f1": mean_qa_f1,
                "mean_qa_em": mean_qa_em,
                "mean_layer_iou": (
...
def aggregate_routing_analysis(records: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for record in records:
        ...
        qa_f1_total += float(record.get("qa_scores", {}).get("f1", 0.0))
        qa_em_total += float(record.get("qa_scores", {}).get("em", 0.0))
        ...
        batch_records.append(
            {
                ...
                "qa_f1": float(record.get("qa_scores", {}).get("f1", 0.0)),
                "qa_em": float(record.get("qa_scores", {}).get("em", 0.0)),
            }
        )
    ...
    aggregate_record = {
        ...
        "mean_qa_f1": qa_f1_total / len(records),
        "mean_qa_em": qa_em_total / len(records),
