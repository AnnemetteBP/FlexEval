...
def parse_expert_label_overrides(raw_labels: list[str]) -> dict[int, str]:
    labels: dict[int, str] = {}
...
def expert_labels_for_model(model_name: str, num_experts: int, overrides: dict[int, str]) -> dict[int, str]:
    labels = {
        idx: label for idx, label in enumerate(expert_tick_labels_for_model(model_name, num_experts, multiline=True))
    }
...
def expert_label(expert_idx: int, labels: dict[int, str]) -> str:
    return labels.get(expert_idx, f"Expert {expert_idx}")
...
def top1_top2_confusion(records: list[dict], language: str | None = None) -> np.ndarray | None:
    ...
    max_expert_idx = -1
    flattened_pairs: list[tuple[list[int], list[int]]] = []
...
def build_summary_rows(
    ...
    run_label: str,
    expert_label_overrides: dict[int, str],
) -> list[dict]:
    rows: list[dict] = []
    for dataset_name in dataset_names:
        ...
        for display_language, raw_language in dataset_language_groups(dataset_name, reference_records):
            for model_name in model_names:
                ...
                if matrix is None:
                    continue
                expert_labels = expert_labels_for_model(model_name, matrix.shape[0], expert_label_overrides)
...
def plot_confusion_matrices(
    ...
    fig, axes = plt.subplots(
        len(row_specs),
        len(model_names),
        figsize=(5.9 * len(model_names), 3.65 * len(row_specs)),
        squeeze=False,
    )
    ...
    for row_idx, (dataset_name, display_language, raw_language) in enumerate(row_specs):
        for col_idx, model_name in enumerate(model_names):
            ...
            if show_cbar and fig.axes:
                ...
                colorbar_ax.set_ylabel("P(top2 | top1)", fontweight=FONT_WEIGHTS["label"], fontsize=FONT_SIZES["axis_label"])
            panel_title = model_display_name(model_name)
            if row_idx == 0:
                panel_title = f"{panel_title} | {dataset_display_name(dataset_name)}"
            ax.set_title(
                panel_title,
                fontweight=FONT_WEIGHTS["title"],
            ...
            if row_idx == len(row_specs) - 1:
                ...
                ax.set_xlabel("Top-2 Expert", fontweight=FONT_WEIGHTS["label"], fontsize=FONT_SIZES["axis_label"], labelpad=6)
            else:
                ax.set_xticklabels([])
                ax.set_xlabel("")
    ...
    fig.subplots_adjust(left=0.095, right=0.96, bottom=0.09, top=0.86, wspace=0.18, hspace=0.10)
...
def main() -> int:
    args = parse_args()
    expert_label_overrides = parse_expert_label_overrides(args.expert_label)
    ...
    output_path = plot_confusion_matrices(
        ...
        expert_label_overrides=expert_label_overrides,
