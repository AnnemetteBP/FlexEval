...
import numpy as np
import pandas as pd
from flexolmo_analysis.toolkit.plotting.style import dataset_display_name, model_display_name, style_axis_labels, style_axis_title, style_legend, style_suptitle
...
def plot_correlation_by_dataset(correlations: pd.DataFrame, output_root: Path, model_names: list[str]) -> list[Path]:
    ...
    for (dataset_name, phase), subset in correlations.groupby(["dataset_name", "phase"], sort=True):
        ...
        for ax, (confidence_metric, outcome_metric, title) in zip(axes.flat, metric_specs):
            ...
            style_axis_labels(ax, "Layer", "Spearman r")
        ...
        style_suptitle(fig, f"{dataset_display_name(dataset_name)} | {phase.capitalize()} routing confidence vs outcome", y=0.96)
...
def plot_bucket_summary(buckets: pd.DataFrame, output_root: Path, model_names: list[str]) -> list[Path]:
    ...
    for (dataset_name, phase), subset in buckets.groupby(["dataset_name", "phase"], sort=True):
        ...
        for ax, (metric_name, title) in zip(axes[0], plot_specs):
            ...
            style_axis_title(ax, title)
            style_axis_labels(ax, "Confidence bucket (low → high)", "Mean value")
            ax.tick_params(labelsize=10)
        ...
        style_suptitle(fig, f"{dataset_display_name(dataset_name)} | {phase.capitalize()} margin-confidence buckets", y=0.94)
