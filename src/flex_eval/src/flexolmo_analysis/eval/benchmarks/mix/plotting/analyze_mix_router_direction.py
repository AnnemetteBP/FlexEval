...
from flexolmo_analysis.toolkit.plotting.style import FONT_SIZES, FONT_WEIGHTS, dataset_display_name, expert_tick_labels_for_model, expected_num_experts_for_model, model_display_name, style_axis_labels, style_axis_title, style_legend
...
def plot_weight_heatmaps(
    ...
    for row_idx, layer_idx in enumerate(layer_ids):
        for col_idx, model_name in enumerate(model_names):
            ...
            style_axis_title(ax, f"{model_display_name(model_name)} | L{layer_idx}")
            style_axis_labels(ax, "Expert", "")
            ...
            ax.set_ylabel("")
    fig.text(0.028, 0.5, "Expert", rotation=90, va="center", ha="center", fontsize=FONT_SIZES["axis_label"], fontweight=FONT_WEIGHTS["label"])
...
def plot_top1_expert_heatmaps(
    ...
    for ax, model_name in zip(axes, model_names):
        model_num_experts = expected_num_experts_for_model(model_name) or num_experts
        matrix = np.zeros((len(datasets) * len(layer_ids), model_num_experts), dtype=np.float32)
        ylabels = []
        for dataset_idx, dataset_name in enumerate(datasets):
            for layer_pos, layer_idx in enumerate(layer_ids):
                ...
                if subset:
                    counts = np.zeros(model_num_experts, dtype=np.float32)
                    for record in subset:
                        expert_idx = int(record["top1_aligned_expert"])
                        if expert_idx < model_num_experts:
        ...
        style_axis_title(ax, model_display_name(model_name))
        style_axis_labels(ax, "Top-1 aligned expert", "")
        ax.set_xticks(range(model_num_experts))
        tick_labels = expert_tick_labels_for_model(model_name, model_num_experts, multiline=True)
