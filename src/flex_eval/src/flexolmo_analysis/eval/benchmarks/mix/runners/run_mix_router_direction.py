...
def compute_alignment_scores(state: torch.Tensor, weight_matrix: torch.Tensor, metric: str) -> torch.Tensor:
    ...
    assert_all_finite("alignment_state", state)
    assert_all_finite("alignment_weights", weights)
    if metric == "dot":
        scores = torch.mv(weights, state)
        assert_all_finite("alignment_scores", scores)
        return scores
    if metric == "cosine":
        ...
        scores = torch.mv(normalized_weights, normalized_state)
        assert_all_finite("alignment_scores", scores)
        return scores
...
def main() -> int:
    ...
    for layer_idx in selected_layers:
        ...
        assert_all_finite(f"router_weights.layer_{layer_idx}", router_weights[f"layer_{layer_idx}_weights"])
    ...
    for dataset_entry in manifest_entries:
        ...
        for example in examples:
            ...
            for layer_idx in selected_layers:
                ...
                assert_all_finite(f"state.layer_{layer_idx}", state)
                assert_all_finite(f"scores.layer_{layer_idx}", scores)
                ...
                assert_all_finite(f"routing_info.layer_{layer_idx}", routing_info)
        ...
        assert_all_finite(f"dataset_records[{dataset_name}]", dataset_records)
        manifest = {
            "dataset_name": dataset_name,
