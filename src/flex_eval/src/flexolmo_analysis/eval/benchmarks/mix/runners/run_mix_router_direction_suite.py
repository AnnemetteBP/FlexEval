...
RUNNER_PATH = PROJECT_ROOT / "eval" / "benchmarks" / "mix" / "runners" / "run_mix_router_direction.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the mix router-direction suite across multiple checkpoints.")
    parser.add_argument("--config", required=True, help="Path to the router-direction suite config JSON.")
...
def main() -> int:
    ...
    if not models:
        raise ValueError("No enabled models were defined in the router-direction config.")
    ...
    output_root = runtime.get("output_root", str(PROJECT_ROOT / "eval_results" / "mix" / "router_direction" / "a4"))
    ...
    for model in models:
        command = [
            ...
            "--alignment-metric",
            str(runtime.get("alignment_metric", "cosine")),
    ...
    manifest_path = Path(output_root) / "router_direction_suite_commands.json"
