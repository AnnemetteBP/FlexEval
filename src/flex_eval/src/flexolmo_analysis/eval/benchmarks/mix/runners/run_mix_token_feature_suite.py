...
RUNNER_PATH = PROJECT_ROOT / "eval" / "benchmarks" / "mix" / "runners" / "run_mix_token_features.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the mix token-feature suite across multiple checkpoints.")
    parser.add_argument("--config", required=True, help="Path to the token-feature suite config JSON.")
...
def main() -> int:
    ...
    if not models:
        raise ValueError("No enabled models were defined in the token-feature config.")
    ...
    representation_sources = shared.get("representation_sources", "pre_router,hidden_state")
    datasets = shared.get("datasets", [])
    datasets_arg = ",".join(str(name) for name in datasets) if datasets else None
    output_root = runtime.get("output_root", str(PROJECT_ROOT / "eval_results" / "mix" / "token_features" / "a4"))
    ...
    manifest_path = Path(output_root) / "token_feature_suite_commands.json"
