...
RUNNER_PATH = PROJECT_ROOT / "eval" / "benchmarks" / "mix" / "runners" / "run_mix_weight_analysis.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the mix weight-analysis suite across multiple checkpoints.")
    parser.add_argument("--config", required=True, help="Path to the weight-analysis suite config JSON.")
...
def main() -> int:
    ...
    if not models:
        raise ValueError("No enabled models were defined in the weight-analysis config.")
    ...
    output_root = runtime.get("output_root", str(PROJECT_ROOT / "eval_results" / "mix" / "weight_analysis" / "a4"))
    ...
    manifest_path = Path(output_root) / "weight_analysis_suite_commands.json"
