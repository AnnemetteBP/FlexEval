...
from flexolmo_analysis.toolkit.utils import (
...
def parse_hidden_state_layers(raw_value: str | None, *, num_hidden_layers: int | None = None) -> list[int] | None:
    ...
    if normalized == "all":
        if num_hidden_layers is None:
            raise ValueError("`num_hidden_layers` is required to resolve layer preset `all`.")
...
def normalize_examples(tokenizer, records: list[dict], dataset_name: str, dataset_entry: dict) -> list[dict]:
    prompting_config = dict(dataset_entry.get("prompting", {}))
    euroeval_config = dict(prompting_config.get("euroeval_compat", {}))
    use_euroeval_compat = bool(euroeval_config.get("enabled", False))

    if not use_euroeval_compat:
        return [
            normalize_example(
                tokenizer=tokenizer,
                record=record,
    ...
    few_shot_enabled = bool(euroeval_config.get("few_shot", True))
    num_few_shot_examples = int(euroeval_config.get("num_few_shot_examples", 4))
    normalized_base_records = [
        normalize_example(
            tokenizer=tokenizer,
            record=record,
            dataset_name=dataset_name,
    ...
    return normalize_examples_for_euroeval_compat(
        tokenizer,
        normalized_base_records,
